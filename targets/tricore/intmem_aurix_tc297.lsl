...
derivative my_tc29x extends tc29x
{
    ...
    memory pspr0 (tag="on-chip")
    {
        ...
        size = 32k;
        exec_priority=17;
        map (dest=bus:tc0:fpi_bus, dest_offset=0xc0000000, size=32k, priority=4,exec_priority=16);
    ...
    memory pflash2 (tag="on-chip")
    {
        mau = 8;
        type = reserved rom;
        size = 2M;
        map cached(dest=bus:sri, dest_offset=0x80400000, size=2M);
        map not_cached(dest=bus:sri, dest_offset=0xA0400000, size=2M, reserved);
    }
    memory pflash3 (tag="on-chip")
    {
        mau = 8;
        type = reserved rom;
        size = 2M;
        map cached(dest=bus:sri, dest_offset=0x80600000, size=2M);
        map not_cached(dest=bus:sri, dest_offset=0xA0600000, size=2M, reserved);
    }
