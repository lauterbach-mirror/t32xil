...
% --------------------------------------------------------------------------------
% $Id: trace32_settings.m 7348 2023-10-13 10:31:30Z csax $
...
function  cfg = trace32_settings()
    ...
    cfg.T32_Executable          = 't32mppc';
    cfg.T32_Startupscript       = 'trace32_mpc8610_startup.cmm';
...
function hooks = SetPracticeCallbacks()
    ...
    %% ### Start: User adaptable values ###
    hooks.PreInit  = {'callback', 'interface'};  % Triggered once immediately before the simulation is started
    hooks.PostTerm = {'', ''};  % Triggered once after the simulation has terminated
    % ### End: User adaptable values ###
end
